                ...
                try:
                    ...
                    if xlsx_path and os.path.exists(xlsx_path):
                        await context.bot.send_document(
                            ...
                            caption="🏆 Here are your TOP 10 ranked companies as an Excel sheet."
                        )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    telegram_id = update.effective_user.id
    if has_user_completed(telegram_id):
        await update.message.reply_text("⚠️ You have already completed a session! Only one run is allowed per user.")
        return
    ...
    if session["state"] == "qa_flow":
        ...
        ## --- More robust end-of-QA detection using fuzzy matching ---
